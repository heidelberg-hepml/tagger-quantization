...
def plot_mixer(cfg, plot_path, title, plot_dict):
    ...
    if cfg.plotting.histograms and cfg.evaluate:
        ...
        with PdfPages(out) as file:
            ...
            data = [
                np.log(plot_dict["results_test"]["raw"]["truth"]),
                np.log(plot_dict["results_train"]["raw"]["truth"]),
                np.log(plot_dict["results_test"]["raw"]["prediction"]),
            ]
            plot_histograms(
                file,
                data,
    ...
    if cfg.plotting.delta and cfg.evaluate:
        ...
        with PdfPages(out) as file:
            delta_test = (
                plot_dict["results_test"]["raw"]["prediction"]
                - plot_dict["results_test"]["raw"]["truth"]
            ) / plot_dict["results_test"]["raw"]["truth"]
            delta_train = (
                plot_dict["results_train"]["raw"]["prediction"]
                - plot_dict["results_train"]["raw"]["truth"]
            ) / plot_dict["results_train"]["raw"]["truth"]

            # determine 1% largest amplitudes
            scale = plot_dict["results_test"]["raw"]["truth"]
    ...
    if cfg.plotting.delta_prepd and cfg.evaluate:
        ...
        with PdfPages(out) as file:
            delta_test = (
                plot_dict["results_test"]["prepd"]["prediction"]
                - plot_dict["results_test"]["prepd"]["truth"]
            ) / plot_dict["results_test"]["prepd"]["truth"]
            delta_train = (
                plot_dict["results_train"]["prepd"]["prediction"]
                - plot_dict["results_train"]["prepd"]["truth"]
            ) / plot_dict["results_train"]["prepd"]["truth"]

            # determine 1% largest amplitudes
            scale = plot_dict["results_test"]["prepd"]["truth"]
            largest_idx = round(0.01 * len(scale))
            sort_idx = np.argsort(scale)
            largest_min = scale[sort_idx][-largest_idx - 1]
            largest_mask = scale > largest_min
