...
from .perm_equiv_layers import (
    eops_1_to_2,
    eops_2_to_2,
    eops_2_to_1,
    eops_2_to_0,
)  # , eset_ops_3_to_3, eset_ops_4_to_4, eset_ops_1_to_3, eops_1_to_2
...
class Eq2to0(nn.Module):
    def __init__(
        ...
        if factorize:
            self.coefs00 = nn.Parameter(
                torch.normal(
                    0,
                    np.sqrt(1.0 / self.basis_dim),
                    (in_dim, self.basis_dim),
                    device=device,
                    dtype=dtype,
                )
            )
            self.coefs01 = nn.Parameter(
                torch.normal(
                    0,
                    np.sqrt(1.0 / self.basis_dim),
            ...
            self.coefs = nn.Parameter(
                torch.normal(
                    0,
                    np.sqrt(4.0 / (in_dim * self.basis_dim)),
    ...
    def forward(self, inputs, mask=None, nobj=None, irc_weight=None):
        ...
        for i, char in enumerate(self.config):
            if char in ["s", "m", "x", "n"]:
                ...
                raise ValueError(
                    "args.config must consist of the following letters: smxnSMXN",
                    self.config,
                )
            ops.append(op)
...
class Eq1to2(nn.Module):
    def __init__(
        ...
        for i, char in enumerate(config):
            if char in ["M", "X", "N"]:
                self.alphas[i] = nn.Parameter(
                    torch.zeros(1, in_dim, 5, 1, 1, device=device, dtype=dtype)
                )
            elif char == "S":
                self.alphas[i] = nn.Parameter(
                    torch.zeros(1, in_dim, 5, 1, 1, device=device, dtype=dtype)
        ...
        if factorize:
            ...
            )  # Replace 1. with 2. when using ELU/GELU, leave 1. for LeakyReLU
            self.coefs11 = nn.Parameter(
                torch.normal(
                    0,
                    np.sqrt(1.0 / in_dim),
            ...
            self.coefs = nn.Parameter(
                torch.normal(
                    0,
                    np.sqrt(2.0 / (in_dim * self.basis_dim)),
                    (in_dim, out_dim, self.basis_dim),
                    device=device,
                    dtype=dtype,
                )
...
class Eq2to1(nn.Module):
    def __init__(
        ...
        for i, char in enumerate(config):
            if char in ["M", "X", "N"]:
                self.alphas[i] = nn.Parameter(
                    torch.zeros(1, in_dim, 5, 1, device=device, dtype=dtype)
                )
            elif char == "S":
                self.alphas[i] = nn.Parameter(
                    torch.zeros(1, in_dim, 5, 1, device=device, dtype=dtype)
        ...
        for i, char in enumerate(self.config):
            if char.lower() in ["s", "m", "x", "n"]:
                op = self.ops_func(
                    ...
                    aggregation=d[char.lower()],
                    weight=irc_weight,
                    skip_order_zero=False if i == 0 else True,
...
class Net2to2(nn.Module):
        ...
        self.eq_layers = nn.ModuleList(
            [
                Eq2to2(
                    ...
                    eq_out_dims[i],
                    ops_func,
                    activate_agg=activate_agg,
                    activate_lin=activate_lin,
                    activation=activation,
                    config=config,
    ...
    def forward(self, x, mask=None, nobj=None, softmask_ir=None, irc_weight=None):
        ...
        ), "Input dimension of Net2to2 doesn't match the dimension of the input tensor"
