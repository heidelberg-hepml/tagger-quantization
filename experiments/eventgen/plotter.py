...
def plot_losses(exp, filename, model_label):
    with PdfPages(filename) as file:
        ...
        plot_metric(
            file,
            [torch.stack(exp.grad_norm_train).cpu()],
            "Gradient norm",
            logy=True,
        )
        plot_metric(
        ...
        plot_metric(
            file,
            [torch.stack(exp.grad_norm_net).cpu()],
        ...
        plot_metric(
            file,
            [
                exp.train_metrics["reg_collinear"],
                exp.train_metrics["reg_coplanar"],
                exp.train_metrics["reg_lightlike"],
            ],
        ...
        plot_metric(
            file,
            [
                np.cumsum(exp.train_metrics["reg_collinear"]),
                np.cumsum(exp.train_metrics["reg_coplanar"]),
                np.cumsum(exp.train_metrics["reg_lightlike"]),
            ],
...
def plot_classifier(exp, filename, model_label):
    with PdfPages(filename) as file:
        ...
        # probabilities
        data = [
            exp.classifiers.results["logits"]["true"],
            exp.classifiers.results["logits"]["fake"],
        ]
        simple_histogram(
            file,
            data,
        ...
        # weights
        data = [
            exp.classifiers.results["weights"]["true"],
            exp.classifiers.results["weights"]["fake"],
        ]
        simple_histogram(
            file,
            data,
        ...
        plot_roc(
            file,
            exp.classifiers.results["tpr"],
            exp.classifiers.results["fpr"],
            exp.classifiers.results["auc"],
        )
        # calibration curve
        plot_calibration(
            file,
            exp.classifiers.results["prob_true"],
            exp.classifiers.results["prob_pred"],
        )
...
def plot_fourmomenta(exp, filename, model_label, weights, mask_dict):
    ...
    with PdfPages(filename) as file:
        ...
        for channel in range(num_components):
            ...
            train = extract(exp.data_raw["trn"])
            test = extract(exp.data_raw["tst"])
...
def plot_preprocessed(exp, filename, model_label, weights, mask_dict):
    with PdfPages(filename) as file:
        ...
        for channel in range(
            exp.data_prepd["trn"].reshape(exp.data_prepd["trn"].shape[0], -1).shape[1]
        ):
            train = extract(exp.data_prepd["trn"], channel)
            test = extract(exp.data_prepd["tst"], channel)
            model = extract(exp.data_prepd["gen"], channel)
...
def plot_delta(exp, filename, model_label, weights, mask_dict):
    with PdfPages(filename) as file:
        num_particles = exp.n_hard_particles + exp.cfg.data.n_jets
        ...
        for idx1 in particles:
            for idx2 in particles:
                ...
                model = get_delta_eta(exp.data_raw["gen"])
                plot_histogram(
                    file=file,
                    train=train,
                    test=test,
                ...
                model = get_delta_phi(exp.data_raw["gen"])
                plot_histogram(
                    file=file,
                    train=train,
                    test=test,
                ...
                model = get_delta_r(exp.data_raw["gen"])
                plot_histogram(
                    file=file,
                    train=train,
                    test=test,
